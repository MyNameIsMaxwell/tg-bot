            ...
            try:
                ...
                if chat_type == "user":
                    ...
                    if len(parts) > 1:
                        ...
                        await event.respond(
                            "👋 Привет! Я **SummaryChan** — твоя помощница по сводкам новостей!\n\n"
                            "Я умею читать Telegram-каналы и превращать кучу постов в аккуратную "
                            "сводку с самым важным. Больше никакого бесконечного скролла — только суть! ✨\n\n"
                            "━━━━━━━━━━━━━━━━━━━━\n\n"
                            "📱 **Как начать:**\n\n"
                            "1️⃣ Открой мини-приложение (кнопка внизу 👇)\n\n"
                            "2️⃣ Нажми «Создать шаблон» и укажи:\n"
                            "   • Каналы для чтения (@channel1, @channel2...)\n"
                            "   • Куда отправлять сводку\n"
                            "   • Как часто (каждые 6/12/24 часа)\n\n"
                            "3️⃣ Готово! Я буду присылать тебе дайджест по расписанию 💌\n\n"
                            "━━━━━━━━━━━━━━━━━━━━\n\n"
                            "📌 **Куда могу отправлять сводки:**\n"
                            "• Прямо сюда, в наш чат ✅\n"
                            "• В твой канал (добавь меня админом)\n"
                            "• В группу (тоже нужны права админа)\n\n"
                            "💡 Чтобы добавить канал/группу — напиши:\n"
                            "`/start -1001234567890` (ID канала)\n\n"
                            "━━━━━━━━━━━━━━━━━━━━\n\n"
                            "Приятного чтения! Если что — я всегда тут 🌷"
                        )
                    return
                ...
                if sender and isinstance(sender, TelegramUser):
                    ...
                    await event.respond(
                        f"✅ Ура! {chat_type.capitalize()} зарегистрирован!\n\n"
                        f"📍 ID: `{chat_id_display}`\n\n"
                        f"Теперь открой мини-приложение и выбери этот чат как место для сводок 🌸"
                    )
                else:
        ...
        async def _register_channel_from_private_chat(event, sender, target_id: str):
            ...
            try:
                ...
                try:
                    ...
                    await event.respond(
                        f"😿 Ой, не могу найти чат `{target_id}`...\n\n"
                        "Проверь, пожалуйста:\n"
                        "• ID указан правильно (например, `-1001234567890`)\n"
                        "• Я добавлена в этот канал/группу как админ\n"
                ...
                if target_type == "user":
                    await event.respond(
                        "🤔 Хм, это ID пользователя, а не канала или группы.\n\n"
                        "Мне нужен ID канала (начинается с `-100`) или группы!"
                    )
                    return
                ...
                await event.respond(
                    ...
                    f"📌 **{title}**\n"
                    f"ID: `{chat_id_display}`\n\n"
                    f"Теперь открой мини-приложение и выбери этот чат! 🌸"
                )
            except Exception as e:
                logger.exception("Failed to register channel from PM: %s", e)
                await event.respond(
                    f"😿 Что-то пошло не так: {e}\n\n"
                    "Проверь, что я добавлена в канал/группу как админ, и попробуй снова!"
                )
